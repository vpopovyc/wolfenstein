...
void    set_ray_data(t_ray_info *ray, t_appcore *appcore, int x)
{
	...
	ray->pos.x = appcore->myGame->pos.x;
	ray->pos.y = appcore->myGame->pos.y;
	ray->dir.x = appcore->myGame->dir.x + 
				 appcore->myGame->plane.x * ray->cam_x;
	ray->dir.y = appcore->myGame->dir.y + 
				 appcore->myGame->plane.y * ray->cam_x;
...
{
    mapped_ray->step.x = (ray->dir.x < 0) ? -1 : 1;    
    mapped_ray->step.y = (ray->dir.y < 0) ? -1 : 1;
    mapped_ray->side_dist.x = (ray->dir.x < 0) ?
        				(ray->pos.x - ray->map.x) * ray->delta_dist.x :
        				(ray->map.x + 1.0 - ray->pos.x) * ray->delta_dist.x;
    mapped_ray->side_dist.y = (ray->dir.y < 0) ?
